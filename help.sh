...
help() { #сообщение вывод основной информации
    echo -e "\e[1mUsage:\e[0m $0 [OPTIONS] [DIRECTORY]" #-e - позволяет интерпретировать специальные символы, \e[1m - устанавливает текст в жирный шрифт
    echo                                                #\e[0m - cбрасывает форматирование текста
...
total_size=$(du -sb $directory | cut -f 1)  #размер директории, du - использования дискового пространства файлами и директориями, -s - выводит только общий размер для указанной директории, -b - выводит размер в байтах
...
then
    ...
    tar czfv out.tar.gz ${files_to_compress[@]} #c - создание архива, z - сжатие архива, f - указывает что следующий параметр будет именем файла архива, v - вывод информации о каждом файле
